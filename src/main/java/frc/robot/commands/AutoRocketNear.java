...
public class AutoRocketNear extends CommandGroup {
	public AutoRocketNear(boolean isRightStart) {
		...
		addSequential(new DriveMotionProfile("side_platform_to_near", isRightStart));
		...
		addSequential(new DriveTurnToAngleWithEncoders(isRightStart ? 136 : -136, 12, 8));
		addSequential(new DriveMotionProfile(MotionProfiles.generate1DPF(11.8, 6, 4, 100, false)));
		addSequential(new DriveAutoConfirm());
		addSequential(new HatchGrabberExtend());
		addSequential(new WaitCommand(0.25));
		addSequential(new DriveMotionProfile("wall_reverse_near", isRightStart));
	}
