...
public class DriveVisionAuto extends CommandBase implements PIDOutput, PIDSource {
	PIDController turnController;
	double minTurn = 0.0;//0.18;
	double driveValue;

	public DriveVisionAuto(double driveValue) {
		requires(drive);
	...
	protected boolean isFinished() {
		return ((drive.getRightVelocity() + drive.getLeftVelocity()) / 2) <= 0.5;
	}
	...
	public void pidWrite(double output) {
		...
		drive.arcadeDrive(driveValue, -output, false);
	}
